...
def build_model(alpha_value = 0.001, learning_rate=1):
    ...
    model = keras.models.Sequential([
        ...
        keras.layers.Conv2D(filters=200, kernel_size=[4, 4], padding="valid", activation="relu",
                            kernel_regularizer=tf.keras.regularizers.L2(alpha_value)),
        ...
        keras.layers.Dense(64, activation="relu", kernel_regularizer=tf.keras.regularizers.L1(alpha_value)),
        keras.layers.Dense(3, activation="softmax", kernel_regularizer=tf.keras.regularizers.L1(alpha_value))
    ])
...
param_distribs = {
    ...
    "alpha_value": [4e-3, 7e-3, 1e-4, 4e-4, 7e-4,1e-5]
}
